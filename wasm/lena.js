...
async function runWasm(wasmPath, consoleElement, extraForeignImports, wasmMemoryInterface, intSize = 4) {
	...
	if (exports.step) {
		...
		function step(currTimeStamp) {
			...
			// note this lena addition
			exports.lena_wasm_step(dt, odin_ctx);
...
// lena additions from here on

/**
 * @param {string} wasm_path                - Path to the WASM module to run
 * @param {?HTMLPreElement} console_element - Optional console/pre element to append output to, in addition to the console
 */
function runLena(wasm_path, console_element) {
	const canvas = document.getElementById("lena-canvas");
	const ctx = canvas.getContext("2d");
	ctx.imageSmoothingEnabled = false;

	let audio_ctx = new AudioContext();
	let memory_interface = new WasmMemoryInterface();

	(async () => {
		window.wasm = await window.odin.runWasm(wasm_path, console_element, {
			lena_env: {
				wasm_set_canvas_size: (w, h) => {
					canvas.width  = w;
					canvas.height = h;
				},
				wasm_write_canvas: (pointer, len) => {
					const buffer = new Uint8ClampedArray(memory_interface.memory.buffer, pointer, len);
					const image  = new ImageData(buffer, canvas.width, canvas.height);
					ctx.putImageData(image, 0, 0);
				},
				wasm_play_sound: async (pointer, len, volume, loop, group) => {
					const data = memory_interface.memory.buffer.slice(pointer, pointer + len);
					const buffer = await audio_ctx.decodeAudioData(data);
					const source = audio_ctx.createBufferSource();

					const gain_node = audio_ctx.createGain();
					gain_node.connect(audio_ctx.destination);
					gain_node.gain.setValueAtTime(volume, audio_ctx.currentTime);
					...
					source.onended = () => {
						let s = window.lena.sources;
						let i = s.indexOf(sound_struct);
						s.splice(i, 1);
					}

					window.lena.sources.push(sound_struct);
				},
				wasm_update_volumes: async (pointer, len) => {
					let s = window.lena.sources;
					let volumes = new Float64Array(memory_interface.memory.buffer, pointer, len);
					for (var i = 0; i < s.length; i++) {
						let x = s[i];
						x.gain.gain.setValueAtTime(volumes[x.group], audio_ctx.currentTime);
					}
				},
				wasm_stop_sounds: async (group) => {
					let s = window.lena.sources;
					for (var i = 0; i < s.length; i++) {
						let x = s[i];
						if (group > 0 && group !== x.group) {
							continue
						}
						x.source.stop();
						x.source.disconnect();
